...
class UNetBlockNoise(torch.nn.Module):
    """
    Unified U-Net block with optional up/downsampling and self-attention. Represents
    the union of all features employed by the DDPM++, NCSN++, and ADM architectures.
    ...
    use_apex_gn : bool, optional
        ...
        Need to set this as False on cpu. Defaults to False.
    act : str, optional
        The activation function to use when fusing activation with ConditionalGroupNorm. Defaults to None.
    fused_conv_bias: bool, optional
        A boolean flag indicating whether bias will be passed as a parameter of conv2d. By default False.
    profile_mode:
        A boolean flag indicating whether to enable all nvtx annotations during profiling.
    ...
    def __init__(
        ...
        self.profile_mode = profile_mode
        self.amp_mode = amp_mode
        self.norm0 = ConditionalGroupNorm(
            num_channels=in_channels,
            eps=eps,
            use_apex_gn=use_apex_gn,
            fused_act=True,
        ...
        if self.adaptive_scale:
            self.norm1 = ConditionalGroupNorm(
                num_channels=out_channels,
                eps=eps,
                use_apex_gn=use_apex_gn,
                amp_mode=amp_mode,
        ...
        if self.num_heads:
            self.norm2 = ConditionalGroupNorm(
                num_channels=out_channels,
                eps=eps,
                use_apex_gn=use_apex_gn,
                amp_mode=amp_mode,
    ...
    def forward(self, x, emb, z):
        with nvtx.annotate(
            ...
            if self.adaptive_scale:
                scale, shift = params.chunk(chunks=2, dim=1)
                x = silu(torch.addcmul(shift, self.norm1(x, z), scale + 1))
            else:
                x = self.norm1(x.add_(params), z)
...
class ConditionalGroupNorm(torch.nn.Module):
    """
    Conditional Group Normalization: like GroupNorm but affine parameters
    (scale and bias) are predicted from a conditioning embedding.

    Only difference from GroupNorm: dynamic weight & bias from `cond`.
    """
    ...
    def __init__(
        ...
        self.act = act.lower() if act else act
        self.amp_mode = amp_mode
        self.weight_proj = torch.nn.Linear(cond_dim, num_channels)
        self.bias_proj = torch.nn.Linear(cond_dim, num_channels)

        self.act_fn = self.get_activation_function() if self.fused_act else None

    def forward(self, x, cond):
        assert cond is not None, "ConditionalGroupNorm requires `cond` input"

        weight = self.weight_proj(cond).view(x.shape[0], -1, 1, 1)
        bias = self.bias_proj(cond).view(x.shape[0], -1, 1, 1)

        if self.use_apex_gn:
            x = self.gn(x)  # unchanged
        elif self.training:
            x = torch.nn.functional.group_norm(
                x,
                num_groups=self.num_groups,
    ...
    def get_activation_function(self):
        ...
        return act_map.get(self.act, None)
