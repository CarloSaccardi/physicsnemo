...
class SongUNetCarlo(Module):
    ...
    def __init__(
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc[f"{res}x{res}_down"] = UNetBlockNoise(
                    in_channels=cout, out_channels=cout, down=True, **block_kwargs
                )
                if encoder_type == "skip":
                    self.enc[f"{res}x{res}_aux_down"] = Conv2d(
            ...
            for idx in range(num_blocks):
                ...
                cout = model_channels * mult
                attn = res in attn_resolutions
                self.enc[f"{res}x{res}_block{idx}"] = UNetBlockNoise(
                    in_channels=cin, out_channels=cout, attention=attn, **block_kwargs
                )
        skips = [
            block.out_channels for name, block in self.enc.items() if "aux" not in name
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            res = self.img_shape_y >> level
            if level == len(channel_mult) - 1:
                self.dec[f"{res}x{res}_in0"] = UNetBlockNoise(
                    in_channels=cout, out_channels=cout, attention=True, **block_kwargs
                )
                self.dec[f"{res}x{res}_in1"] = UNetBlockNoise(
                    in_channels=cout, out_channels=cout, **block_kwargs
                )
            else:
                self.dec[f"{res}x{res}_up"] = UNetBlockNoise(
                    in_channels=cout, out_channels=cout, up=True, **block_kwargs
                )
            for idx in range(num_blocks + 1):
                cin = cout + skips.pop()
                cout = model_channels * mult
                attn = idx == num_blocks and res in attn_resolutions
                self.dec[f"{res}x{res}_block{idx}"] = UNetBlockNoise(
                    in_channels=cin, out_channels=cout, attention=attn, **block_kwargs
                )
            if decoder_type == "skip" or level == 0:
                if decoder_type == "skip" and level < len(channel_mult) - 1:
                ...
                self.dec[f"{res}x{res}_aux_norm"] = ConditionalGroupNorm(
                    num_channels=cout,
                    eps=1e-6,
                    use_apex_gn=use_apex_gn,
                    amp_mode=amp_mode,
    ...
    def forward(self, x, noise_labels, class_labels, z, augment_labels=None):
        with nvtx.annotate(
            message="SongUNet", color="blue"
        ) if self.profile_mode else contextlib.nullcontext():
            if self.embedding_type != "zero":
            ...
            for name, block in self.enc.items():
                with nvtx.annotate(
                    ...
                    if "aux_down" in name:
                        ...
                        # For UNetBlocks check if we should use gradient checkpointing
                        if isinstance(block, UNetBlockNoise):
                            if x.shape[-1] > self.checkpoint_threshold:
                                # self.checkpoint = checkpoint?
                                # else: self.checkpoint  = lambda(block,x,emb:block(x,emb))
                                x = checkpoint(block, x, emb, z)
                            else:
                                # AssertionError: Only support NHWC layout.
                                x = block(x, emb, z)
                        else:
...
class SongUNetPosEmbdCarlo(SongUNetCarlo):
    """Extends SongUNet with positional embeddings.
    ...
    def forward(
        ...
        with nvtx.annotate(
            ...
            if self.pos_embd is not None:
                ...
                return super().forward(x, noise_labels, class_labels, z, augment_labels)
